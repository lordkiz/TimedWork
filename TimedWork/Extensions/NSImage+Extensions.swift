...
public enum NSImageColorsQuality: CGFloat {
    case lowest = 50 // 50px
    case low = 100 // 100px
    case high = 250 // 250px
...
fileprivate struct NSImageColorsCounter {
    let color: Double
    let count: Int
...
extension NSImage {
    ...
    public func getColors(quality: NSImageColorsQuality = .high, _ completion: @escaping (NSImageColors?) -> Void) {
    ...
    public func getColors(quality: NSImageColorsQuality = .high) -> NSImageColors? {
        ...
        let sortedColorComparator: Comparator = { (main, other) -> ComparisonResult in
            let m = main as! NSImageColorsCounter, o = other as! NSImageColorsCounter
            if m.count < o.count {
        ...
        while let K = enumerator.nextObject() as? Double {
            ...
            if threshold < C {
                sortedColors.add(NSImageColorsCounter(color: K, count: C))
            }
        }
        sortedColors.sort(comparator: sortedColorComparator)

        var proposedEdgeColor: NSImageColorsCounter
        if 0 < sortedColors.count {
            proposedEdgeColor = sortedColors.object(at: 0) as! NSImageColorsCounter
        } else {
            proposedEdgeColor = NSImageColorsCounter(color: 0, count: 1)
        }
        
        if proposedEdgeColor.color.isBlackOrWhite && 0 < sortedColors.count {
            for i in 1..<sortedColors.count {
                let nextProposedEdgeColor = sortedColors.object(at: i) as! NSImageColorsCounter
        ...
        for color in sortedColors {
            let color = (color as! NSImageColorsCounter).color
